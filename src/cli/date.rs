...
pub fn parse_date_range(start_str: &str, end_str: &str) -> Result<(DateTime<Utc>, DateTime<Utc>)> {
...
pub fn resolve_today() -> Result<TimeRange> {
    let now = Local::now();
...
pub fn resolve_week() -> Result<TimeRange> {
    let now = Local::now();
    let today = now.date_naive();
...
pub fn resolve_time_range_from_args<T: AsRef<str>>(
    first: &str,
    second: Option<&T>,
) -> Result<TimeRange> {
    match first {
        "today" if second.is_none() => resolve_today(),
        "week" if second.is_none() => resolve_week(),
        _ => resolve_explicit_range(first, second.map(|s| s.as_ref())),
    }
}

pub fn resolve_explicit_range(start_str: &str, end_str: Option<&str>) -> Result<TimeRange> {
    let end_str = end_str.context("end date required for explicit range")?;
    let (start, end) = parse_date_range(start_str, end_str)?;
    Ok(TimeRange { start, end })
}

pub fn parse_date_input(input: &str) -> Result<(NaiveDate, Option<NaiveTime>)> {
...
pub fn local_to_utc(naive: NaiveDateTime) -> Result<DateTime<Utc>> {
...
mod tests {
    ...
    fn explicit_date_range_defaults_to_midnight_and_end_of_day() {
        let range =
            resolve_explicit_range("2026-01-25", Some("2026-02-03")).expect("resolve failed");
    ...
    fn explicit_datetime_range() {
        let range = resolve_explicit_range("2026-01-25 14:30", Some("2026-02-03 09:00"))
